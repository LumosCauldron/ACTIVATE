...
int activate() {
	...
	if (status >= Success && numItems) {
		...
		for (long k = 0; k < numItems; k++) {
			...
			printf("window found was %ld \n", window);
			if (XFetchName(display, window, &window_name_return)) {
				printf("found window name for %ld  :  %s  \n", window,
						window_name_return);
			}
...
void getScreen2(const int xx, const int yy, const int W, const int H,
	...
	n = sprintf(buffer, "/tmp/%ld_test%d.xpm", xid, file_counter++);
	...
	n2 = sprintf(filename, "%ld_test%d.png", xid, file_counter++);
